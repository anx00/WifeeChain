    ...
    if (contract && address && duration) {
      try {
        ...
        if (!response.ok) {
          ...
          const apMaxBandwidth = parseInt(apInfo.bandwidthLimit);
          const apMaxDataUsage = parseInt(apInfo.dataLimit);
          console.log("apMaxTime:", apMaxTime, typeof apMaxTime);

          const apDateRegisteredTimestamp = apInfo.timeRegistered;
          //timeregistered is returned in timestamp, so we need to convert it to date
          const apDateRegistered = new Date(
            apDateRegisteredTimestamp * 1000
          ).toLocaleString();
          console.log("AP Registered Date:", apDateRegistered);

          const OwnerRTKQuantity = await contract.methods.getOwnerRTK(mac).call();
          console.log("AP Owner RTK:", OwnerRTKQuantity);

          // make a console log of all the values below
          console.log("intDuration:", intDuration, "apMaxTime:", apMaxTime);
          console.log("intBandwidth:", intBandwidth, "apMaxBandwidth:", apMaxBandwidth);
          console.log("intDataUsage:", intDataUsage, "apMaxDataUsage:", apMaxDataUsage);

          if (intDuration <= apMaxTime && intBandwidth <= apMaxBandwidth && intDataUsage <= apMaxDataUsage) {
            console.log("Connecting to the Access Point...");

            Swal.fire({
              title: "Confirm purchase",
              html: `This connection will cost ${totalPrice} ITK.<br><span style="font-size: 14px;">The owner of this AP has <span style="color: ${OwnerRTKQuantity < 10 ? 'red' : OwnerRTKQuantity >= 11 && OwnerRTKQuantity <= 19 ? 'yellow' : 'green'}">${OwnerRTKQuantity}</span> RTK</span><br><span style="font-size: 14px;">Device registered on ${apDateRegistered}</span>`,
