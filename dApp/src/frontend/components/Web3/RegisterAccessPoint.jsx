...
const RegisterAccessPoint = ({ registerAccessPointAddress }) => {
  ...
  useEffect(() => {
   ...
   const getWifiInfo = async () => {
    ...
    try {
      const wifiInfoResponse = await fetch(
        "/api/device-info",
        {
          method: "POST",
          headers: {
      ...
      if (!wifiInfoResponse.ok) {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: `Error while retrieving WiFi info: ${wifiInfoResponse.status}`,
        });
        return;
      }
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "An error occurred while retrieving WiFi info.",
      });
    } finally {
  ...
  return (
    <Container className="mt-5">
      <Card className="mb-4">
        <Card.Header>
          <h2 className="sfr-rounded" style={{ fontSize: "30px" }}>
            Register your Access Point
          </h2>
        </Card.Header>
        <Card.Body>
          <Form>
            <Form.Group>
              ...
              <Form.Control
                type="text"
                placeholder="Enter the network interface configured in your device"
                value={interfaceName}
                onChange={(e) => setInterfaceName(e.target.value)}
              />
            </Form.Group>
            <Button
              className="mt-4 custom-button2 center-style"
              variant="primary"
              onClick={getWifiInfo}
              disabled={loading} // Disable button while loading
            >
              {loading ? "Loading..." : "Get AP Info"}
            </Button>
            ...
            {isWifiInfoReceived && (
              <div>
                <Form.Group className="mt-4">
                  ...
                  <Form.Control type="text" readOnly className="read-only-input" value={ssid} />
                </Form.Group>
                <Form.Group>
                  <Form.Label>
                    <strong>BSSID</strong>
                  </Form.Label>
                  <Form.Control type="text" readOnly className="read-only-input" value={mac} />
                </Form.Group>
                <Row className="mt-3">
                  <Col>
                    <Form.Group>
                      <Form.Label>
                        <strong>Price (ITK) per minute</strong>
                      </Form.Label>
                      <Form.Control
                        type="number"
                        min="0"
                ...
                <Row className="mt-3">
                  ...
                  <Col>
                    <Form.Group>
                      <Form.Label>
                        <strong>Bandwidth Limit (Mbps)</strong>
                      </Form.Label>
                      <Form.Control
                        type="number"
                        min="0"
