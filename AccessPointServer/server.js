...
app.post('/connect', async (req, res) => {
    ...
    if (msUntilExpiry > 0) {  // Only set the timeout if the expiry time is in the future
        setTimeout(async () => {
        try {
            console.log(`[DISCONNECT] Attempting automatic disconnection for IP: ${clientIp}`);
            // Deny the traffic
            ...
            console.log(`[DISCONNECT] Successfully disconnected IP: ${clientIp}`);
            // Log the automatic disconnection
            let logEntry = `Automatic disconnection at ${new Date().toISOString()}: ${clientIp}\n`;
            fs.appendFileSync('log.txt', logEntry);
        } catch (error) {
            console.error(`[DISCONNECT] Failed to automatically disconnect ${clientIp}: ${error}`);
        }
        }, msUntilExpiry);
    }
...
// ENDPOINT to get the user Token

app.get('/createUserToken', (req, res) => {
  try {
    const token = Array.from({ length: 40 }, () => Math.floor(Math.random() * 10)).join('');
    res.json({ userToken: token });
  } catch (error) {
    console.error('Error al crear el token de usuario:', error);
    res.status(500).json({ error: 'Error al crear el token de usuario' });
  }
});
